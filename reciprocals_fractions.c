...
// To compile:
// gcc -O3 -o reciprocal_fractions reciprocal_fractions.c -lm

// Version for building estimates of counts
// based on:
// 1. terms left
// 2. diff from 1
// 3. last denominator
...
void PrintCSV( uint64_t index ) {
    if ( index < Gterms-3 && G[index].count > 0 ) {
        printf( "%g,%" PRIu64 ",%" PRIu64 ",%" PRIu64 "\n", ((double)G[index].num)/((double)G[index].den), Gterms-index-1, G[index].val, G[index].count ) ;
    }
}
...
void SendResponse( search_more status ) {
    ...
    switch ( status ) {
        case eError:
            fprintf( stderr,"Solutions = %" PRIu64 ", Elapsed = %g sec, %s\n", Gcounter, elapsed, "Error encountered" ) ;
            break ;
        case eYes:
        ...
        case eNo:
            ...
            break ;
        case eTimeout:
            fprintf( stderr,"Solutions = %" PRIu64 ", Elapsed = %g sec, Range [%" PRIu64 " to %" PRIu64 "], %s\n", Gcounter, elapsed, Gfrom, Gto, "Timed out" ) ;
            break ;
    }
}
...
void help() {
    ...
    fprintf(stderr,"\t-s%d\t--sum\ttarget sum (default %d)\n",Gsum,Gsum);
    fprintf(stderr,"\t-q\t--seq\tshow full sequences (default off)\n"); 
